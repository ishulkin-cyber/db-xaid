...
function classifyChange(a: string, b: string): ChangeLevel {
  ...
  const changed = changes.filter((c) => c.added || c.removed).reduce((s, c) => s + c.value.split(/\s+/).filter(Boolean).length, 0);
...
function matchAllThree(doc: ReportSection[], val: ReportSection[], sm: ReportSection[]): MatchedRow[] {
  // Get all unique normalized headings in order
...
function SectionRow({
  ...
  // Determine which two columns are being compared
  ...
  const columns: ColumnKey[] = showDoctor
    ? ["doctor", "validator", "simonmed"]
    : ["validator", "simonmed"];
  ...
  function renderCell(col: ColumnKey) {
    ...
    if (col === leftKey) {
      return <DiffSpan segments={leftSegs} side="left" />;
    }
    return <DiffSpan segments={rightSegs} side="right" />;
  }

  const gridCols = showDoctor ? "grid-cols-3" : "grid-cols-2";
...
export function ReportDiff({ doctorText, validatorText, simonmedText, showDoctor }: ReportDiffProps) {
  const defaultPair: ComparisonPair = showDoctor ? "doctor-validator" : "validator-simonmed";
  const [pair, setPair] = useState<ComparisonPair>(defaultPair);

  const availablePairs: ComparisonPair[] = showDoctor
  ...
  const rows = useMemo(
    () => matchAllThree(showDoctor ? docSections : [], valSections, smSections),
    [docSections, valSections, smSections, showDoctor]
  );

  // Compute stats for the selected pair
  const stats = useMemo(() => {
    const leftKey: ColumnKey = pair === "validator-simonmed" ? "validator" : "doctor";
    const rightKey: ColumnKey = pair === "doctor-validator" ? "validator" : "simonmed";
  ...
  const columnHeaders: { key: ColumnKey; label: string }[] = showDoctor
    ? [
        { key: "doctor", label: "Doctor's Draft" },
  ...
  return (
    <div className="space-y-3">
            ...
            (level) =>
              stats[level] > 0 && (
                <span key={level} className="flex items-center gap-1">
                  <span className={cn("h-2 w-2 rounded-full", changeLevelStyle[level].dot)} />
      ...
      {/* Column headers with highlight for compared pair */}
      ...
      <div className="border rounded-b-lg overflow-hidden -mt-3">
        {rows.map((row, i) => (
          <SectionRow key={i} row={row} pair={pair} showDoctor={showDoctor} />
        ))}
      </div>
    </div>
  );
